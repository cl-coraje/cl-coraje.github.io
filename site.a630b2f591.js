...
if (page === 'contact') document.querySelector('#social-list').innerHTML = content.socials.map(social => {
  const url = safeUrl(social.url);
  const tag = url ? 'a' : 'div';
  const destination = url ? ` href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer"` : '';
  return `<${tag} class="social"${destination}>
    <span class="social-icon" aria-hidden="true">${escapeHtml(social.icon)}</span>
    <span class="social-content"><strong>${escapeHtml(social.name)}</strong><span class="social-account">${escapeHtml(social.account)}</span>${social.handle ? `<span class="social-account">${escapeHtml(social.handle)}</span>` : ''}</span>
    ${url ? '<span class="social-arrow" aria-hidden="true">↗</span>' : ''}
  </${tag}>`;
}).join('');
  updateThemeLabel();
}
